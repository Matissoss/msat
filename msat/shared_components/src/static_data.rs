pub fn serialize(data: [u16; 3]) -> [u8; 6]{
    ...
    [fobin(byte1),fobin(byte2),fobin(byte3),fobin(byte4),fobin(byte5),fobin(byte6)]
}

fn fobin(ba: [bool; 8]) -> u8{
    let mut final_numb = 0u8;
    for (power_of_two, &b) in ba.iter().rev().enumerate(){
        if b{
...
fn slice_boolarray(ba: [bool; 16]) -> ([bool; 8], [bool;8]){
    ...
    for (i, b) in ba.into_iter().enumerate(){
        if i < 8{
            no[i] = b;
        }
        else{
...
fn binrep(numb: u16) -> [bool; 16]{
    ...
    for (i, c) in format!("{}{}", "0".repeat(16-temp.len()), temp).chars().enumerate(){
        to_return[i] = c == '1';
    }
    to_return
}
...
pub fn deserialize(data: [u8; 6]) -> [u16; 3]{
    [not_zero(data[0]) * not_zero(data[1]), not_zero(data[2]) * not_zero(data[3]), not_zero(data[4]) * not_zero(data[5])]
}
